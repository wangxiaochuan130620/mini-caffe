...
int CaffeNetReshape(NetHandle net) {
	API_BEGIN();
	static_cast<caffe::Net*>(net)->Reshape();
	API_END();
}


int CaffeNetNumInputs(NetHandle net) {
	return static_cast<caffe::Net*>(net)->num_inputs();
}

int CaffeNetNumOutputs(NetHandle net) {
	return static_cast<caffe::Net*>(net)->num_outputs();
}


int CaffeNetGetInputBlob(NetHandle net, int index, BlobHandle *blob) {
	API_BEGIN();
	caffe::Blob* blob_ = static_cast<caffe::Net*>(net)->input_blobs()[index];
	*blob = static_cast<BlobHandle>(blob_);
	API_END();
}

int CaffeNetGetOutputBlob(NetHandle net, int index, BlobHandle *blob) {
	API_BEGIN();
	caffe::Blob* blob_ = static_cast<caffe::Net*>(net)->output_blobs()[index];
	*blob = static_cast<BlobHandle>(blob_);
	API_END();
}
